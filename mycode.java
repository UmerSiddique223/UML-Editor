...
public class UseCaseDiagramPanel extends Pane {
    ...
    private void onMousePressedToAddUseCase(MouseEvent mouseEvent) {
        addUserCase(mouseEvent.getSceneX(),  mouseEvent.getSceneY());
        setAddUseCaseMode(false);
    }
    ...
    private void onMouseReleased(MouseEvent mouseEvent) {
        if (tempLine != null) {
            ...
            if (endComponent != null && !selectedComponent.equals(endComponent)) {
                ...
                relationshipLine.setStrokeWidth(10);
    ...
    private void onContextMenuRequested(ContextMenuEvent event) {
        ...
        addActor.setOnAction(e -> addActor(event.getX(), event.getY()));
        addUserCase.setOnAction(e -> addUserCase(event.getX(), event.getY()));
    ...
    private void deleteSelectedComponent(ContextMenuEvent event) {
        ...
        DiagramComponent selected = components.stream()
                .filter(component -> component.shape.contains(event.getX(), event.getY()))
                .findFirst()
                .orElse(null);
    ...
    public void addActor(double x, double y) {
        Circle actor = new Circle(0, 0, 30); // Centered at (0,0) in the StackPane
        actor.setFill(Color.LIGHTGRAY);
        actor.setStroke(Color.BLACK);

        Circle hitbox = new Circle(0, 0, 50); // Larger transparent boundary
        hitbox.setFill(Color.TRANSPARENT); // Make it transparent
        hitbox.setStroke(Color.LIGHTGRAY);

        Text actorText = new Text("Actor");

        StackPane actorContainer = new StackPane(hitbox, actor, actorText);
    ...
    public void addUserCase(double x, double y) {
        ...
        Text useCaseText = new Text("Use Case");
